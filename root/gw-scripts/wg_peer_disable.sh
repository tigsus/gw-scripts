...
if [[ -f "${DISABLED_FILE}" ]]; then
    echo "disabled file already found at ${DISABLED_FILE}"
    exit 0
fi
...
if [ ! $? -eq 0 ]; then
    echo "failed extracting config at $WG_CONF_FILE"
    exit 0
fi

if [[ -z "$EXTRACTED_TEXT" ]]; then
    echo "extracted text is empty string for $ARG_PEER"
    exit 1
fi

echo "$EXTRACTED_TEXT" > "$DISABLED_FILE"

if [ $? -eq 0 ]; then
    echo "success: server config block saved to disabled file at $DISABLED_FILE"
else
    echo "failed disabling extracted text to $DISABLED_FILE"
    exit 1
fi

#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# DELETE FROM SERVER FILE
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if [[ -f "${FILE_USERDEVICE}" ]]; then
    ...
    sed -i "s/^DISABLED=.*/DISABLED=true/" "$FILE_USERDEVICE"
    ...
    if [[ -f "${FILE_USERDEVICE}" ]]; then
        ...
        if ! jq '.disabled = true' "${FILE_USERDEVICE}" > "${FILE_USERDEVICE_TEMP}"; then
